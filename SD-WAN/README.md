...
- Router CE is configured so that when it boots it will automatically try to connect to a controller located in a DC and pull its config.
    - Config includes:
...
- Uses:
    ...
    - DB replication
## SD-WAN components
- <b>vManage:</b> Northbound API/GUI; it is the brain of the operation
    - Design Policies templates to be pushed onto Edge device configurations.
- <b>vSmart:</b> Executes changes ordered by other elements.
- <b>vBond:</b> This is the DMZ where unconfigured devices can reach via ZTP it and be able to authenticate each other. Then vBond orchestrates with the VSmart Controllers to send the config back to the node.
    - Use:
        - Provisioning
        - Onboarding
- <b>vAnalytics:</b> Collects stats and displays them to see all network resources health and additional information like live traffic, etc.
...
## SD-WAN API 
This API provides different exclusive ways to pull data and push data on the system
...
If a user is successfully authenticated, the response body is empty and a valid session cookie is set in the response `(set-cookie: JSESSIONID=)`.
If a user is unauthenticated, the response body contains an HTML login page with tag in it.
API client should check the response body to identify whether the authentication is successful or not. This is the behavior of our application server.
...
- Destroying the session:
    - Create `POST` request
    - URL: `https://{vmanage-ip-address}/logout?nocache={random-number}`
    - Headers: 
        - `Cookie: JESSIONID={session hash id}`
